...
describe('evaluation', () => {
  ...
  describe('evaluateAnswer', () => {
    it('returns true if string passed matches exactly the correct answer', () => {
      expect(
        h.evaluateAnswer('this 15 the answer', correctAnswers, options),
      ).toStrictEqual(true)
    })

    it(
      'returns true if string passed matches the correct ' +
        'answer after filtering out allowed characters',
      () => {
        const answer = 'this$ 1%5 the answer-'
        const correctAnswer = 'this 15 the answer'

        expect(h.evaluateAnswer(answer, correctAnswer, options)).toStrictEqual(
          true,
        )
      },
    )
    ...
    it('fails if the two strings have 1 string difference', () => {
      expect(
        h.evaluateAnswer('this 15 the answe', correctAnswers, options),
      ).toStrictEqual(false)
      expect(
        h.evaluateAnswer('this 15 th answer', correctAnswers, options),
      ).toStrictEqual(false)
      expect(
        h.evaluateAnswer('ths 15 the answer', correctAnswers, options),
      ).toStrictEqual(false)
    })
  })
  ...
  describe('evalAgainstAllAnswers', () => {
    ...
    it('returns true if answer matches at least one of the correct answers', () => {
      ...
      expect(
        h.evalAgainstAllAnswers(answer, correctAnswers, options),
      ).toStrictEqual(true)
    })
    ...
    it('returns true if an array of answers match with an array of the correct answers', () => {
      ...
      expect(
        h.evalAgainstAllAnswers(answers, correctAnswers, options),
      ).toStrictEqual(true)
    })
    ...
    it(
      'returns true if string passed matches one of the ' +
