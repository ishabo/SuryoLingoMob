...
const allLessons = (skills: skills.ISkill[]) =>
  [].concat.apply(
    [],
    skills.map((skill: skills.ISkill) => skill.lessons),
  )
...
export const getComingSoonSkills = (state: IInitialState) =>
  skills.selectors.getComingSoonSkills(state.skills)

export const getPublishedSkills = (state: IInitialState) =>
  skills.selectors.getPublishedSkills(state.skills)
...
export const getSkillsByUnit = (unit: number) => (
  state: IInitialState,
): skills.ISkill[] => skills.selectors.getSkillsByUnit(unit)(state.skills)
...
export const getSkillInProgress = (state: IInitialState): skills.ISkill => {
  ...
  return state.skills.find(
    (skill: skills.ISkill) =>
      skill.lessons.find(
        (skillLesson: skills.ILesson) => skillLesson.id === lessonId,
      ) !== void 0,
  )
}
...
export const numOfTimesLessonInProgressPassed = (
  state: IInitialState,
): number => {
...
export const userIsTester = (state: IInitialState): boolean =>
  state.profile.isTester !== null

export const isLoading = (state: IInitialState): boolean => state.api.loading

export const getLatestException = (
  state: IInitialState,
...
export const calcTotaluserXp = (state: IInitialState): number =>
  state.skills.reduce(
    (total: number, skill: skills.ISkill): number => total + skill.totalSkillXp,
    0,
  )

export const getSkillIcon = (state: IInitialState) => (
  icon: string,
  size: assets.TImageSizes,
): assets.ISkillIcon => assets.selectors.getSkillIcon(icon, size)(state.assets)

export const getPhrases = (state: IInitialState): questions.IPhrase[] => {
  const phrases = state.questions.onGoing.map(
    ({ phrase, translation, soundFiles }) => {
      const sentence = {
        raw: phrase,
        hintified: hintify(
          phrase,
          state.dictionaries,
          getActiveCourse(state).targetLanguage.shortName,
        ),
      }
...
export const isUserInLeaderboard = (state: IInitialState): boolean =>
  ...
  !!state.leaderboard.topUsers.find(
    (user: leaderboard.ILeaderboardUser) => user.id === state.profile.id,
  )
...
export const getDeviceSpecificSettings = (
  state: IInitialState,
): IDeviceSettings => state.settings[Platform.OS]
...
export const shouldUpdateApp = (state: IInitialState): boolean => {
  ...
  if (device) {
    ...
    return (
      currentVersion < device.version && device.update.since <= currentVersion
    )
  }
